...
const char* filename = "/home/sstamal/ch12n12.txt";
...
void employee::write()
   ...
   ouf.open( filename, ios::app | ios::out | ios::in );
   if(!ouf)
      { cout << "\nCan't open file\n"; return; }
   ...
   while(true){
      ...
      switch(ch){
         ...
         case 'r':            //read all employees from file
            employee::read(); break;
         case 'f':
            cout << '\nwrite a number for find the person';
            cin >> number;
            employee::find( number );
